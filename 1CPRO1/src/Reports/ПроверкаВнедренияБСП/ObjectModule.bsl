...
Перем ПроверяемаяПодсистема, КаталогВыгрузки, СоответствиеТерминов, ДеревоПодсистем, ОтборПоПодсистемам,
	...
	ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации, ИмяВидаОбъектаВВыгрузкеФайлов, ЕстьОшибкиСоставаРазделителей, КонфигурацияEDT;
...
Функция ИсправляемыеОшибки()
	...
	Список.Добавить("СоставРолиOData",
		НСтр("ru = 'Интерфейс OData. В роли УдалениеДанныхOData ошибки в составе прав'"));
				...
				Если ЭлементСостава.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать
					...
					ИначеЕсли СтрНачинаетсяС(ЭлементСостава.Метаданные.ПолноеИмя(), "РегистрСведений.") Тогда
						Если ЭлементСостава.Метаданные.ОсновнаяФормаЗаписи <> Неопределено Тогда
							ФормыДляОбработки.Добавить(ЭлементСостава.Метаданные.ОсновнаяФормаЗаписи);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Процедура Подключаемый_БизнесПроцессыИЗадачи_ПроверитьВнедрение()
	...
	СравнитьТипы(ПроверяемыеОбъектыАдресацииЗадач, НСтр("ru = 'Предмет заданий'"));
...
Процедура Подключаемый_УчетОригиналовПервичныхДокументов_ПроверитьВнедрение()
	...
	|	УчетОригиналовПервичныхДокументовКлиент.СписокВыбор(Поле.Имя,ЭтотОбъект,Элементы.Список);
	|КонецЕсли";
	ПроверитьНаличиеВставкиКода(ПараметрыПроверки);
	...
	ПараметрыПроверки.СтрокаКода        = 
	"УчетОригиналовПервичныхДокументов.ОбновитьКомандыСостоянияОригинала(ЭтотОбъект,Элементы.Список)";
	ПроверитьНаличиеВставкиКода(ПараметрыПроверки);
	...
	ПроверяемыеВызовы = Новый Массив;
	ПроверяемыеВызовы.Добавить("УчетОригиналовПервичныхДокументов.ПриСозданииНаСервере_ФормаСписка");
				...
				Если Отсутствующие.Количество() <> 0 Тогда
					...
					Если ПустаяСтрока(ОписаниеТипа) Тогда
						ОписаниеПроверяемогоТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Тип %1'"), Тип.ПолноеИмя());
					Иначе
						ОписаниеПроверяемогоТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1 %2'"), ОписаниеТипа, Тип.ПолноеИмя());
					КонецЕсли;	
...
//   ИмяРоли       - полное имя роли - строка
//   Исправлять    - Ложь - только тестировать, Истина - исправлять
//
	...
	Пока Истина Цикл
		...
		Если Не ЭтоПоле Тогда // Это команда.
			Если (ПропускаемыйОбъект = Неопределено Или Не ПропускаемыйОбъект["View"])
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" лишнее право Просмотр.'"), ИмяРеквизита);
		...
		Если ПраваНаПоле["View"] <> Истина Тогда
			Если Исправлять Тогда
				УстановитьСвойство(ПравоОбъектаПросмотр, СвойствоЗначениеПрава(), Истина);
				ИмеютсяИзмененныеРоли = Истина;
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" без права Просмотр.'"), ИмяРеквизита);
		...
		Если ПраваНаПоле["Edit"] <> Истина Тогда
			Если Исправлять Тогда
				УстановитьСвойство(ПравоОбъектаРедактирование, СвойствоЗначениеПрава(), Истина);
				ИмеютсяИзмененныеРоли = Истина;
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" без права Редактирование.'"), ИмяРеквизита);
...
Процедура ПроверитьНерекомендуемыйСоставРолей()
	...
	Для Каждого Файл Из Файлы Цикл
		...
		Исправлять = Ложь;
		Если ЭтоДемоБСП() И ИсправлятьОшибку("НерекомендуемыйСоставРолей") Тогда
			Исправлять = Истина;
		КонецЕсли;
...
Процедура СравнитьРольСЭталоном(ИмяРоли, МакетыРоли, ПропускатьНовыеПрава, ЕстьОшибки)
	...
	Если ЭтоДемоБСП Тогда
		...
		ШаблонИсправленияОшибки = Символы.ПС
			+ НСтр("ru = 'Для автоматического обновления макета нужно запустить отчет ПроверкаВнедренияБСП на версии платформы, совпадающей с режимом совместимости конфигурации
			...
			|
			|Важно: Отчет не проверяет корректность прав, а только обновляет поставляемый шаблон прав БСП, поэтому сначала необходимо исправить ошибки по правам в АПК.'");
...
Процедура ДобавитьОшибкуПравНаРеквизитыРоли(Знач ОбъектМетаданных, КраткоеОписаниеОшибки, ПодробноеОписаниеОшибки, ПодсистемаБСП = Неопределено, Критичная = Ложь)
	...
	Если ЭтоНоваяСтрока Тогда
		ПодробноеОписаниеОшибки = "1.1. См. пункт 1 стандарта ""Проверка прав доступа"" https://its.1c.ru/db/v8std#content:737:hdoc
...
Функция ШаблонПутиКФайлуМодуляКоманды()
	...
	Если КонфигурацияEDT Тогда 
		ШаблонСтроки = "[КаталогВыгрузки][ИмяБазовогоТипа]\[ИмяОбъекта]\Commands\[ИмяКоманды]\CommandModule";
	Иначе
		ШаблонСтроки = "[КаталогВыгрузки][ИмяБазовогоТипа]\[ИмяОбъекта]\Commands\[ИмяКоманды]\Ext\CommandModule";
	КонецЕсли;
